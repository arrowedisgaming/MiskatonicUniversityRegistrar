...
### Added
- **Investigator portraits.** Optional portrait URL field in the wizard backstory step (with live preview). Saved investigators show the portrait in the sheet header with a pencil edit button that works in read and play mode — paste a link to an image hosted elsewhere; we don't store files. Portraits also appear on shared sheets, the investigators list, and campaign vitals cards.
- **Campaigns.** A Keeper creates a campaign at `/campaigns/new`, mints an unguessable join link (`/campaigns/join/<token>` — 16-char nanoid mirroring the existing investigator-share token pattern), and players follow it to bring one of their finished investigators in. The campaign page resolves keeper vs player role and renders accordingly: keepers see a play dashboard of per-player vitals (HP / MP / Sanity / Luck + most recent roll), a unified roll log, a sheets tab showing every member, and an inventory panel to push items into a specific player's character. Players see the same dashboard read-only plus their roll log. Live updates use polling every 5 seconds and pause when the tab is hidden. Live reference — a character keeps its owner and rolls in real time; leaving the campaign is soft, so rejoining the same investigator reactivates the prior membership row without losing history.
- **Unified campaign roll log.** Every percentile check, weapon damage, SAN check, SAN loss, development roll, and generic dice roll a player makes is mirrored from their sheet into a new `campaign_rolls` table — one hook at `appendPlayHistory` in `src/routes/sheet/[id]/+page.svelte` covers every call site. Cursor pagination uses the table's autoincrement integer `id` (nanoids are random and unsuitable as an ordering cursor). The local `playRollHistory` is still the source of truth on the player's screen, so a network blip during emission costs at most one missed entry in the keeper view.
- **Keeper inventory push.** From the campaign page a Keeper can push weapons, items, or a cash delta into any active player's investigator. The endpoint accepts a delta, not a full character — the server reads the investigator row fresh, applies the delta, re-validates the full character with the same Zod + game-rule check as `PUT /api/investigators/[id]`, and writes with an `expectedUpdatedAt` precondition. On a 409 the keeper UI retries up to three times with the refreshed timestamp. Each push also appends a synthetic `keeperInventory` entry into `campaign_rolls` so the unified log shows the change.

### Security
- The campaign share token uses the same 16-char nanoid generator and rotation semantics as the existing investigator share link, with a retry loop on the (astronomical) `share_id` unique-index collision case so the failure mode is a clean 500 instead of a raw SQLite constraint error. Players cannot read the share token of a campaign they are in — the `GET /api/campaigns/:id` response strips it for non-keepers. Cross-campaign tampering is blocked at the inventory endpoint: even with a guessed `memberId`, the route confirms the member belongs to the URL's campaign before applying the delta. Player-submitted roll entries cannot spoof a different investigator — both the `investigatorId` AND the `investigatorName` snapshot stored on each `campaign_rolls` row are resolved from the caller's active membership server-side (joined against the `investigators` table) rather than trusted from the request body. The append-roll endpoint validates entries against the same strict per-kind Zod schemas the character-save path uses — no `.passthrough()`, no silently-stored extra fields, oversized strings / arrays / dice counts rejected outright. The synthetic `keeperInventory` entry kind is excluded from the player-emit union so only the server-side `addItemsToMember` path can produce one.
- One user can have at most one active investigator per campaign, enforced by partial unique indexes (`campaign_members_user_active_uq` and `campaign_members_investigator_active_uq`, both `WHERE left_at IS NULL`). The app-level check in `joinCampaign` still runs first for friendly 409 messages, but the DB constraint closes the TOCTOU race between concurrent join requests and the read-then-insert pattern. Soft-leave + re-join still works because historical (left) rows are excluded from the partial index.

### Fixed
- Keeper inventory push could break after the first dashboard poll because `updatedAt` was a `Date` on initial SvelteKit load but a string after `fetch(...).json()` polling. The wire contract is now uniform — `DashboardMemberVitals.updatedAt`, `lastRollAt`, and `CampaignRollRow.createdAt` are all ms-since-epoch numbers, so `member.updatedAt` works the same on first paint and on every subsequent poll. The push handler also wraps its body in try/finally so a thrown error can never leave the form's `busy` flag stuck on.
- Player auto-save (every roll, every HP/SAN tick) could silently overwrite a keeper-inventory push that landed between the player's last load and their next save — the keeper-side write had optimistic concurrency but the player's `PUT /api/investigators/[id]` did not. The PUT now accepts an optional `expectedUpdatedAt` precondition; the sheet page sends it on every save and handles 409 by refreshing the loader base (overlay $state survives) and retrying once. Symmetric optimistic concurrency on both sides closes the only remaining lost-update window.
- The campaign Sheets tab linked every member's vitals card to `/sheet/{id}`, but that route only loads investigators owned by the caller. Non-owners hitting the link got a 404. The dashboard payload now carries a per-row `belongsToCaller` flag (computed server-side from the membership row's userId), and the link is only rendered when it points at the caller's own sheet.

### Migration
- Drizzle migration `0003_campaigns.sql` adds `campaigns`, `campaign_members`, and `campaign_rolls`. The rolls table uses `integer PRIMARY KEY AUTOINCREMENT` for monotonic cursor support; the FK to users / investigators on `campaign_rolls` is `ON DELETE SET NULL` with a denormalised `investigator_name` snapshot so the log stays readable after account or investigator deletion. Migration `0004_campaign_member_active_unique.sql` adds the two partial unique indexes that codify the one-active-investigator-per-user-per-campaign rule at the DB layer. CI applies both to the remote `miskatonic-db` automatically on deploy.
