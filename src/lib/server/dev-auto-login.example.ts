 ...
 *
 * On first request without a session cookie, this handle:
 *   - Looks up (or creates) a fixed dev user `dev@local` in the SQLite users table
 *   - Mints an Auth.js-compatible JWT signed with DEV_AUTH_SECRET
 *   - Sets it as the `authjs.session-token` cookie
 *   - Hands off to the real authHandle, which sees a valid session
 */
...
export const devAutoLoginHandle: Handle = async ({ event, resolve }) => {
	...
	// Mirror auth.ts:36 secret resolution — Auth.js verifies with whatever it
	// finds at AUTH_SECRET first, falling back to DEV_AUTH_SECRET only when
	// unset. Diverging here would silently produce JWTs the verifier rejects.
	...
	// Set the cookie on the response so the browser persists it for next time...
	...
	// ...AND inject it into the request the downstream authHandle reads from.
	// Auth.js parses the session cookie from `event.request.headers.cookie`, not
	// from `event.cookies`, so without this line the very first request still
	// 401s and redirects to /login. We rebuild the Request with augmented
	// headers and replace event.request (writable at runtime despite TS types).
